...
# FASE 1 — TCP
# ─────────────────────────────────────────────
def make_tcp_worker(timeout: float, concurrency: int):
    ...
    def worker(chunk):
        try:
            return asyncio.run(run_chunk(chunk))
...
# FASE 2 — HTTP
# ─────────────────────────────────────────────
def make_http_worker(timeout: float, concurrency: int, test_url: str):
    async def http_check(proxy_url: str, session, sem: asyncio.Semaphore):
...
def main():
    ...
    # ── Fase 1: TCP ────────────────────────────────────────────────────
    tcp_worker = make_tcp_worker(args.timeout, args.concurrency)
    tcp_alive  = run_phase(
    ...
    if args.tcp_only or not tcp_alive:
        ...
        http_worker  = make_http_worker(args.http_timeout, args.http_concurrency, args.http_url)
